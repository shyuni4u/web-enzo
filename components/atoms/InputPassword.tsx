...
import { BsFillEyeFill, BsFillEyeSlashFill, BsExclamationCircle } from 'react-icons/bs';
import { FormContext } from './Form';

type MInputPasswordProps = React.InputHTMLAttributes<HTMLInputElement> & {
  /**
...
export type InputPasswordProps = React.DetailedHTMLProps<MInputPasswordProps, HTMLInputElement>;

type StyledInputPasswordProps = {
  _size: 'medium' | 'large';
  focus: boolean;
  label?: string;
  variant: 'outline' | 'ghost';
...
`;
const StyledInputPasswordWrapper = styled.div<StyledInputPasswordProps>`
  position: relative;
  box-sizing: border-box;
...
`;
const StyledInputPassword = styled.input<StyledInputPasswordProps>`
  flex: 1 1 auto;
  line-height: 1.5;
...
`;
type StyledInputPasswordMessageProps = {
  error?: boolean;
};
const StyledInputPasswordMessage = styled.span<StyledInputPasswordMessageProps>`
  font-size: ${({ theme }) => theme.fontSizes.f12};
...
export const InputPassword = forwardRef((props: InputPasswordProps, ref: React.Ref<HTMLInputElement>) => {
    ...
    return (
      <StyledWrapper style={props.style}>
        ...
        >
          ...
          {showPassword && <BsFillEyeFill onClick={() => setShowPassword(false)} />}
          {!showPassword && <BsFillEyeSlashFill onClick={() => setShowPassword(true)} />}
          {trailingIcon && <StyledTrailingIcon className={trailingIconClassName}>{trailingIcon}</StyledTrailingIcon>}
        </StyledInputPasswordWrapper>
        {message && (
          <StyledInputPasswordMessage error={error}>
            <BsExclamationCircle />
            {message}
          </StyledInputPasswordMessage>
        )}
      </StyledWrapper>
    );
  }
