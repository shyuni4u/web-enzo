...
type MInputNumberProps = React.InputHTMLAttributes<HTMLInputElement> & {
  /**
...
export type InputNumberProps = React.DetailedHTMLProps<MInputNumberProps, HTMLInputElement>;
...
`;
const StyledInputNumberWrapper = styled.div<StyledInputNumberProps>`
  position: relative;
  box-sizing: border-box;
...
`;
const StyledInputNumber = styled.input<StyledInputNumberProps>`
  flex: 1 1 auto;
  line-height: 1.5;
...
`;
type StyledInputNumberMessageProps = {
  error?: boolean;
};
const StyledInputNumberMessage = styled.span<StyledInputNumberMessageProps>`
  font-size: ${({ theme }) => theme.fontSizes.f12};
...
export const InputNumber = forwardRef((props: InputNumberProps, ref: React.Ref<HTMLInputElement>) => {
  const size = props.sizeMoreh || 'medium';
  const variant = props.variant || 'outline';
  const label = props.label || undefined;
  const placeholder = props.placeholder || 'Input number ...';
  ...
  if (isView) {
    ...
    return (
      <StyledWrapper style={props.style}>
        ...
        {message && (
          <StyledInputNumberMessage error={error}>
            <BsExclamationCircle />
            {message}
          </StyledInputNumberMessage>
        )}
      </StyledWrapper>
    );
  }
